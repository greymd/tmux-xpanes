...
Some of you prefer to use `-d` and `-s` all the time.
Just add below statement to your shell's startup file (i.e `~/.bashrc`) like this.

```sh
alias xpanes='xpanes -d -s'
```
...
## Same as:
##    xpanes -B "set {}" -c 'echo $1-$2-$3' 'A B C' 'E F G' 'H I J' 'L M N'
```
